...
class Admin extends CI_Controller {
	...
	public function index() {
		...
		$q		=	"SELECT a.nip, concat(if(a.gdp='','',concat(a.gdp,' ')),a.nama,if(a.gdb='','',concat(', ',a.gdb)))as nama,
			concat(a.tmlhr, date_format(a.tglhr,'%d-%m-%Y')) as ttl, a.idgolrupkt,b.pangkat,b.golru, if(a.idjenjab in('20','30','40'), c.jab_utuh,
			if(a.idjenjab='2', j.jabfung,o.jabfungum))as jabatan, c.path_short as unit_kerja
			from `tb_01`a	
		...
		$nip 	= $this->session->userdata('admin_id');
		$a['datpil'] = $this->db->query("$q where nip='$nip'")->row();		
...
public function kp() {
		...
		$q3						=	"SELECT a.skpd,a.golkp,a.jenkp, b.nip,a.nama,b.nama_file,b.path,a.id FROM t_arsip_digital b
...
// nama zipfile yang akan dibuat 
 //proses membuat zip file 
				...
				$jfile= $this->db->query("SELECT nama_file, path from t_arsip_digital where nip='$nip'")->result();
...
exit();
		...
		if ($mau_ke == "del") {
			...
			$jpen	= $this->db->query("select * from a_arsip")->result();
				foreach ($jpen as $e) {
				$kd_file	= $e->kode;
			...
			if($this->upload->do_upload($kd_file)){
				...
				$fx				= $nip."_".$kd_file;
				$file1			= $fx.$file_ext;
				$new_file		= $file_path.$kd_file."/".$file1;
				rename($file_asli,$new_file);
				
			$jum	=	$this->db->query("SELECT * FROM t_arsip_digital where nip='$nip' and id_doc='$id_doc'")->num_rows();
			if($jum > '0'){
				$this->db->query("update t_arsip_digital set create_date=date('d/m/Y h:i:s') where nip='$nip' and id_doc='$id_doc'");
			}else{
				$this->db->query("insert into t_arsip_digital (id_doc,nip,nama_file,path,create_date)values('$id_doc','$nip','$file1','$new_file',date('d/m/Y h:i:s'))");
			}	
			}
				}
//			$a['datpil']	= $this->db->query("SELECT * FROM t_kp WHERE id = '$idp'")->row();
...
public function get_jabfungum() {
	...
	public function do_login() {
		...
		$q_cek	= $this->db->query("SELECT * FROM tb_01 WHERE nip = '".$u."' AND nip = '".$p."'");
        ...
        if($j_cek == 1) {
            $data = array(
                    'admin_id' => $d_cek->nip,
                    'admin_user' => $d_cek->nama,
                    'admin_nama' => $d_cek->nama,
                    'admin_ta' => $ta,
                    'admin_level' => $d_cek->idjenjab,
