...
<div class="navbar navbar-inverse"style="background-color:#3CB371;">
	...
	?>
	<form action="<?=base_URL()?>admin/pensiun/add2" method="post" accept-charset="utf-8" enctype="multipart/form-data">
	...
	<div class="col-lg-6">	
		...
		<tr><td width="30%">Jenis Pensiun</td><td><select name="jenpens" class="form-control" style="width: 150px" required><option value="<?php echo $jenpens;?>"> - Jenis Pensiun - </option>
			<?php
				$jpen	= $this->db->query("SELECT * FROM a_jenpens")->result();
				
				foreach ($jpen as $e) {
					if ($e->idjenpens == $jenpens) {
		...
		<tr><td width="30%">golpens</td><td><select name="golpens" class="form-control" style="width: 150px" required><option value="<?php echo $golpens;?>"> - Golongan Pensiun - </option>
			<?php
				$gol2	= $this->db->query("SELECT * FROM a_golruang")->result();
				
				foreach ($gol2 as $d) {
					if ($d->idgolru == $golpens) {
